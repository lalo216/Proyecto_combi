...
/// Los métodos CRUD individuales son poco propensos a fallar, por lo que no es una crud en si todavia, sorry.
class InstalaDB {
  InstalaDB._();
  ...
  Future<void> _crearTablas(Database db) async {
    ...
    await db.execute('''
       CREATE TABLE IF NOT EXISTS paradas (
        ...
        foreign key rutanom references (rutas)nombre
      )
    ''');
    // Okay shit this is actually complicated, currently we have no stops table as they were just an outcome of users favorites, this guy wants us to 
    // Draw the routes by first adding the stops and havign the program draw between them, okay, then every combi is dependent on having distinc stops to draw thier routes.
    await db.execute('''
  ...
  Future<void> _sembrarDatosEjemplo(Database db) async {
    await db.execute('''
      INSERT INTO rutas (nombre, numero, paradas)
      VALUES
        ("Virgen, 2, 10")
    ''');
    
    await db.execute('''
      INSERT INTO paradas (rutaid, numero, coordenadaY, coordenadaX)
      VALUES
        ("Virgen",  "01", 15, 5),
        ("Ocotlan", "12",  8, 12)
    ''');
